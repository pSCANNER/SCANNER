            ...
            if ( app )
            {
                ...
                for (var k=0; k < app.initialPopulation.models.length && (i < 0 || j < 0); ++k)
                {
                    if ( app.initialPopulation.models[k].attributes.type === 'endDate' )
                        j = k;
                    else if ( app.initialPopulation.models[k].attributes.type === 'startDate' )
                        i = k;
                }
                var highValue = {type:'TS', value: app.initialPopulation.models[i].attributes.name, 'inclusive?':true, 'derived?':false};
                var lowValue = {type:'TS', value: app.initialPopulation.models[j].attributes.name, 'inclusive?':true, 'derived?':false};
